...
export const SafeguardingView: React.FC<SafeguardingViewProps> = ({
  ...
  if (!isAuthorized) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center space-y-4">
        ...
        <h2 className="font-display text-2xl font-bold text-[#17212B]">Restricted Safeguarding Access</h2>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8 animate-in fade-in duration-300 text-left">
      ...
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#17212B] text-[#FAF9F6] p-6 sm:p-8 rounded-3xl shadow-xl">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <ShieldAlert className="w-6 h-6 text-rose-400" />
            <h1 className="font-display text-2xl sm:text-3xl font-bold">Safespace Trust & Safeguarding</h1>
          </div>
          <p className="text-xs text-[#E3E2DE] leading-relaxed max-w-2xl">
        ...
        <div className="flex items-center gap-2 bg-[#17212B] p-2.5 rounded-2xl border border-[#59636B] shrink-0">
          <Lock className="w-4 h-4 text-[#123B5D]" />
          <div className="text-[11px]">
            <div className="font-bold text-[#E3E2DE]">Active Reviewer:</div>
            <div className="text-[#FAF9F6] font-mono">{currentUserName} ({currentUserRole})</div>
          </div>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex flex-wrap gap-2 border-b border-[#E3E2DE] pb-2">
        <button
          onClick={() => setActiveTab('CASES')}
          className={`px-4 py-2.5 rounded-xl font-bold text-xs transition-all flex items-center gap-2 ${
            activeTab === 'CASES'
              ? 'bg-[#123B5D] text-[#FAF9F6] shadow-md'
              : 'bg-[#F3F1EC] text-[#59636B] hover:bg-[#E3E2DE]'
          }`}
        >
          <ShieldAlert className="w-4 h-4 text-[#FAF9F6]" />
        ...
        >
          <HelpCircle className="w-4 h-4 text-[#FAF9F6]" />
        ...
        >
          <History className="w-4 h-4 text-[#FAF9F6]" />
      ...
      {activeTab === 'CASES' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="bg-white p-4 rounded-2xl border border-[#E3E2DE] shadow-2xs space-y-1">
              <div className="text-[10px] uppercase font-bold text-[#59636B]">Total Cases</div>
              <div className="font-display text-2xl font-bold text-[#17212B]">{cases.length}</div>
              <div className="text-[10px] text-[#59636B]">All registered incidents</div>
            </div>

            <div className="bg-white p-4 rounded-2xl border border-[#E3E2DE] shadow-2xs space-y-1">
              <div className="text-[10px] uppercase font-bold text-[#59636B]">Critical / High Severity</div>
              <div className="font-display text-2xl font-bold text-rose-700">
            ...
            <div className="bg-white p-4 rounded-2xl border border-[#E3E2DE] shadow-2xs space-y-1">
              <div className="text-[10px] uppercase font-bold text-[#59636B]">Open Human Reviews</div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-[#E3E2DE] shadow-2xs space-y-1">
              <div className="text-[10px] uppercase font-bold text-[#59636B]">Actioned & Resolved</div>
              <div className="font-display text-2xl font-bold text-[#123B5D]">
                {cases.filter(c => c.status === 'ACTIONED' || c.status === 'RESOLVED').length}
              </div>
              <div className="text-[10px] text-[#123B5D] font-semibold">Decided & Documented</div>
            </div>
          </div>

          {/* Cases Stream Table */}
          <div className="bg-white rounded-3xl p-6 border border-[#E3E2DE]/90 shadow-md space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-display text-xl font-bold text-[#17212B]">Active Safeguarding Review Cases</h3>
              <span className="text-xs text-[#59636B] font-mono">RBAC Access Recorded</span>
            </div>

            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="border-b border-[#E3E2DE] text-[#59636B] uppercase tracking-wider font-semibold text-[10px]">
                ...
                <tbody className="divide-y divide-[#F3F1EC] text-[#59636B]">
                  {cases.map((c) => (
                    <tr key={c.id} className="hover:bg-[#FAF9F6] transition-colors">
                      <td className="py-3.5 px-3 font-mono font-bold text-[#17212B]">{c.id}</td>
                      ...
                      <td className="py-3.5 px-3 font-bold text-[#17212B]">{c.riskCategory}</td>
                      <td className="py-3.5 px-3">
                        <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-[#123B5D]/10 text-[#0D2A42]">
                          {c.stage}
                        </span>
                      </td>
                      <td className="py-3.5 px-3 text-[#59636B]">{c.assignedReviewerName || 'Unassigned'}</td>
                      <td className="py-3.5 px-3 text-[#59636B]">{new Date(c.createdAt).toLocaleTimeString()}</td>
                      <td className="py-3.5 px-3 text-right">
                        <button
                          onClick={() => handleOpenCaseDetail(c)}
                          className="px-3 py-1.5 bg-[#123B5D] text-[#FAF9F6] rounded-lg text-xs font-bold hover:bg-[#0D2A42] transition-colors flex items-center gap-1.5 ml-auto"
                        >
      ...
      {activeTab === 'AUTHORITIES' && (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-display text-xl font-bold text-[#17212B]">Configured Authorities & Specialist Organisations</h2>
            ...
            <button
              ...
              className="px-4 py-2.5 bg-[#123B5D] text-[#FAF9F6] rounded-xl font-bold text-xs hover:bg-[#0D2A42] transition-colors flex items-center gap-2"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {authorities.map((org) => (
              <div key={org.id} className="bg-white p-5 rounded-2xl border border-[#E3E2DE] shadow-2xs space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <span className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-[#F3F1EC] text-[#59636B]">
                      Category: {org.category}
                    </span>
                    <h3 className="font-display text-lg font-bold text-[#17212B] mt-1">{org.name}</h3>
                  </div>
                ...
                <p className="text-xs text-[#59636B] leading-relaxed">{org.protocolNotes}</p>

                <div className="pt-2 border-t border-[#F3F1EC] text-xs space-y-1">
                  <div className="flex items-center gap-2 text-[#17212B] font-semibold">
                    <Phone className="w-3.5 h-3.5 text-[#59636B]" />
                    <span>{org.contactPhone}</span>
                  </div>
                  {org.contactEmail && (
                    <div className="flex items-center gap-2 text-[#59636B]">
                      <Mail className="w-3.5 h-3.5 text-[#59636B]" />
      ...
      {activeTab === 'RESOURCES' && (
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#E3E2DE]/90 shadow-md space-y-6">
          <div className="space-y-1">
            <h2 className="font-display text-2xl font-bold text-[#17212B]">Safety Directory & Emergency Resources</h2>
            <p className="text-xs text-[#59636B]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-5 rounded-2xl bg-amber-50/60 border border-amber-200/80 space-y-2">
              <div className="font-display font-bold text-amber-950 text-base">Nigeria Suicide Prevention & Crisis Hotline</div>
            ...
            <div className="p-5 rounded-2xl bg-rose-50/60 border border-rose-200/80 space-y-2">
              <div className="font-display font-bold text-rose-950 text-base">Gender-Based Violence Crisis Response</div>
            ...
            <div className="p-5 rounded-2xl bg-[#F3F1EC]/60 border border-[#123B5D]/20/80 space-y-2">
              <div className="font-display font-bold text-[#0D2A42] text-base">National Emergency Response</div>
              <p className="text-xs text-[#123B5D] leading-relaxed">
                Toll-free national police, ambulance, and fire dispatch service.
              </p>
              <div className="pt-2 font-mono font-bold text-[#0D2A42] text-sm flex items-center gap-2">
                <Phone className="w-4 h-4 text-[#123B5D]" />
                <span>112 / 199</span>
              </div>
            </div>

            <div className="p-5 rounded-2xl bg-[#FAF9F6] border border-[#E3E2DE] space-y-2">
              <div className="font-display font-bold text-[#17212B] text-base">Safespace Peer Support Policy</div>
      ...
      {/* TAB 4: AUDIT TRAIL */}
      {activeTab === 'AUDIT' && (
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#E3E2DE]/90 shadow-md space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-display text-xl font-bold text-[#17212B]">Safeguarding Access & Action Audit Trail</h2>
              <p className="text-xs text-[#59636B]">
                Immutable system audit log recording every view, stage transition, decision, and escalation.
              </p>
            </div>
            <span className="text-xs font-mono text-[#123B5D] font-bold bg-[#123B5D]/10 px-3 py-1 rounded-full">
              SECURE LOGGED
            </span>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-[#F3F1EC] text-[#59636B]">
                {auditLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-[#FAF9F6] font-mono text-[11px]">
                    <td className="py-3 px-3 text-[#59636B]">{log.id}</td>
                    <td className="py-3 px-3 font-bold text-[#17212B]">{log.caseId}</td>
                    <td className="py-3 px-3 font-sans font-semibold text-[#17212B]">{log.actorName}</td>
                    <td className="py-3 px-3 font-sans text-[#59636B]">{log.actorRole}</td>
                    <td className="py-3 px-3 font-sans font-bold text-[#0D2A42]">{log.action}</td>
                    <td className="py-3 px-3 text-[#59636B]">{new Date(log.timestamp).toLocaleTimeString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {selectedCase && (
        <div className="fixed inset-0 z-50 bg-[#17212B]/70 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-3xl p-6 sm:p-8 shadow-2xl border border-[#E3E2DE] relative space-y-6 text-left">
            
            {/* Modal Header */}
            <div className="flex items-start justify-between border-b border-[#E3E2DE] pb-4">
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-mono text-xs font-bold bg-[#F3F1EC] px-2.5 py-0.5 rounded-md text-[#59636B]">
                    {selectedCase.id}
                  </span>
                ...
                <h3 className="font-display text-2xl font-bold text-[#17212B] mt-1">
              ...
              <button
                ...
                className="px-3 py-1 bg-[#F3F1EC] hover:bg-[#E3E2DE] rounded-lg text-xs font-bold text-[#59636B]"
              >
                Close
              </button>
            </div>
            ...
            {selectedCase.aiSummaryHint && (
              <div className="p-3 bg-[#FAF9F6] border border-[#E3E2DE] rounded-xl text-xs text-[#59636B] space-y-1">
                <div className="font-bold text-[#17212B] text-[11px] uppercase tracking-wider">AI Advisory Pre-Filter Note</div>
                <p className="italic text-[#59636B]">{selectedCase.aiSummaryHint}</p>
              </div>
            )}
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-xl border text-left transition-all ${
                      transitionStage === st.id
                        ? 'bg-[#123B5D] text-[#FAF9F6] border-[#123B5D] shadow-sm font-bold'
                        : 'bg-[#FAF9F6] text-[#59636B] border-[#E3E2DE] hover:bg-[#F3F1EC]'
                    }`}
                  >
                    <div className="text-xs">{st.label}</div>
                    <div className={`text-[10px] mt-0.5 ${transitionStage === st.id ? 'text-[#FAF9F6]/80' : 'text-[#59636B]'}`}>
                      {st.desc}
                    </div>
                  </button>
                ))}
            ...
            {/* Human Decision Form */}
            <div className="space-y-4 pt-2 border-t border-[#E3E2DE]">
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full p-3 bg-[#FAF9F6] border border-[#E3E2DE] rounded-xl text-xs text-[#17212B] focus:ring-2 focus:ring-[#123B5D] outline-hidden"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2.5 bg-[#FAF9F6] border border-[#E3E2DE] rounded-xl text-xs font-semibold text-[#17212B]"
                  >
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2.5 bg-[#FAF9F6] border border-[#E3E2DE] rounded-xl text-xs text-[#17212B]"
                  />
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-3 bg-[#123B5D] hover:bg-[#0D2A42] text-[#FAF9F6] rounded-xl font-bold text-xs shadow-md transition-all cursor-pointer"
              >
            ...
            <div className="p-4 bg-[#17212B] text-[#FAF9F6] rounded-2xl space-y-3">
              <div className="font-display font-bold text-sm text-[#FAF9F6] flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                <div>
                  <label className="text-[10px] text-[#59636B] uppercase font-bold block mb-1">
                    Select Configured Specialist Authority
                  </label>
                  <select
                    value={selectedAuthorityId}
                    onChange={(e) => setSelectedAuthorityId(e.target.value)}
                    className="w-full px-3 py-2 bg-[#17212B] border border-[#59636B] rounded-xl text-xs text-amber-100 font-semibold"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-[#17212B] border border-[#59636B] rounded-xl text-xs text-[#E3E2DE]"
                  />
                </div>
              </div>
              ...
              <button
                ...
                className="px-4 py-2 bg-amber-400 text-[#17212B] rounded-xl font-bold text-xs hover:bg-amber-300 transition-colors cursor-pointer"
              >
            ...
            <div className="space-y-2 pt-2 border-t border-[#E3E2DE]">
              ...
              <div className="bg-[#FAF9F6] rounded-xl p-3 max-h-36 overflow-y-auto space-y-1 font-mono text-[11px] text-[#59636B]">
                {selectedCase.documentationNotes?.map((note, idx) => (
                  <div key={idx} className="border-b border-[#E3E2DE]/60 pb-1 last:border-0">
                    {note}
                  </div>
                ))}
              </div>
      ...
      {showAddAuthModal && (
        <div className="fixed inset-0 z-50 bg-[#17212B]/70 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-lg rounded-3xl p-6 sm:p-8 shadow-2xl border border-[#E3E2DE] relative space-y-5 text-left">
            <div className="flex items-center justify-between border-b border-[#E3E2DE] pb-3">
              ...
              <button
                ...
                className="text-xs text-[#59636B] font-bold hover:text-[#17212B]"
              >
                Cancel
              </button>
            </div>
            ...
            <form onSubmit={handleAddAuthority} className="space-y-4 text-xs">
              <div>
                <label className="font-bold text-[#17212B] block mb-1">Organisation Name *</label>
                <input
                  type="text"
                  value={newAuthName}
                  ...
                  required
                  className="w-full p-2.5 bg-[#FAF9F6] border border-[#E3E2DE] rounded-xl text-[#17212B]"
                />
              </div>

              <div>
                <label className="font-bold text-[#17212B] block mb-1">Primary Risk Category</label>
                <select
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="font-bold text-[#17212B] block mb-1">Contact Phone *</label>
                  <input
                    type="text"
                    value={newAuthPhone}
                ...
                <div>
                  <label className="font-bold text-[#17212B] block mb-1">Contact Email</label>
                  <input
                    type="email"
                    value={newAuthEmail}
              ...
              <div>
                <label className="font-bold text-[#17212B] block mb-1">Protocol & Escalation Notes</label>
                <textarea
                  value={newAuthNotes}
              ...
              <button
                type="submit"
                className="w-full py-3 bg-[#123B5D] text-[#FAF9F6] rounded-xl font-bold hover:bg-[#0D2A42] transition-colors cursor-pointer"
              >
